...
class Spider:
    ...
    class Lesson:
        ...
        def __init__(self, xkflag, name, code, teacher_name, time,  location, credit, totalNum, availableNum):
            self.xkflag = xkflag
            self.name = name
            self.code = code
            ...
            self.availableNum = availableNum

        def show(self):
            print('  xkflag:' + str(self.xkflag) +'  name:' + str(self.name) + '  code:' + str(self.code) + '  teacher_name:' + str(self.teacher_name) + '  time:' + str(self.time)+ '  location:' + str(self.location)+ '  credit:' + str(self.credit)+ '  totalNum:' + str(self.totalNum)+ '  availableNum:' + str(self.availableNum))
    ...
    def __get_code(self):
        ...
        res = requests.get(self.__real_base_url + 'CheckCode.aspx')
    ...
    def login(self, uid, password):
        ...
        while True:
            ...
            if self.__real_base_url != 'http://202.199.155.37/':
                print(self.__real_base_url + 'default2.aspx')
    ...
    def __get_lessons(self, soup):
        ...
        for lesson_tag in lesson_tag_list:
            ...
            xkflag = td_list[0].input['name']
            name = td_list[2].string
            code = td_list[3].string
            teacher_name = td_list[4].string
            try:
                Time = td_list[5]['title']
            except:
                Time = ''
            location = td_list[6].string
            credit = td_list[7].string
            totalNum = td_list[10].string
            availableNum = td_list[11].string
            lesson = self.Lesson(xkflag, name, code, teacher_name, Time, location, credit, totalNum, availableNum)
    ...
    def run(self,uid,password):
        ...
        if self.login(uid, password):
            ...
            # print('请输入想选的课的id，id为每门课程开头的数字,如果没有课程显示，代表公选课暂无')
            #select_id = input()
            select_id = config['courseId']
            for i in range(len(lesson_list)):
                if lesson_list[i].code ==select_id:
                    lesson_list=lesson_list[i]
                    break
